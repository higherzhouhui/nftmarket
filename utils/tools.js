...
function formatAddress(address, long) {
	let add = ''
	if (!address) {
		add = ''
	} else {
		...
		add = address.slice(0, strNum) + '...' + address.slice(address.length - strNum)
	}
	return add
}

export { getNetworkType, throttle, countTimeDown, theNextDayTime, debounce, formatAddress };
